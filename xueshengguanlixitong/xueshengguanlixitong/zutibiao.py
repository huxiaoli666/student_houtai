...
class headsearch(View):
    def post(self,request):
        ...
        sql3 = "select * from tests"
        cursor.execute(sql3)
        types = cursor.fetchall()
        return render(request,"testsinfo.html",{"data":result,"grades":grades,"parts":parts,"types":types,"gid":gid,"pid":pid,"tid":tid})

class zutiTable(View):
    def get(self,request):
        page = request.GET.get("page") if request.GET.get("page") else 0
        page = int(page)
        num = 3
        cursor = db.cursor()
        sql = "select tests.id,types.tname as tnames,part.pname as pnames,grade.gname as gnames,title,tests.`opts`,result from tests LEFT JOIN types on types.tid = tests.tid LEFT JOIN part on part.pid=tests.pid LEFT JOIN grade on tests.gid=grade.gid limit %s,%s"
    ...
    def post(self,request):
        pass
class zutiTableAdd(View):
    def get(self,request):
        # cursor = db.cursor()
        # sql = "select tests.id,types.tname as tnames,part.pname as pnames,grade.gname as gnames,title,tests.`opts`,result from tests LEFT JOIN types on types.tid = tests.tid LEFT JOIN part on part.pid=tests.pid LEFT JOIN grade on tests.gid=grade.gid"
        # cursor.execute(sql)
        # result = cursor.fetchall()
    ...
    def post(self,request):
        ...
        print("------------",gid,pid,tid,title,opts,result)
        print(type(title))
        print(type(opts))
        print(type(result))
